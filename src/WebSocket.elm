...
@docs Message, listen, keepAlive, send
...
{-| Message is either Text or Binary
-}
type Message
...
-}
listen : String -> (Message -> msg) -> Sub msg
listen url tagger =
...
type alias SubsDict msg =
    Dict.Dict String (List (Message -> msg))
...
onSelfMsg router selfMsg state =
    case selfMsg of
        Receive name llmsg ->
            let
                -- NOTE: The type Message is duplicated in WebSocket.LowLevel and needs to be converted here. This is done so that users of this module don't need to go deep into LowLevel.
                msg =
                    case llmsg of
                        WS.Binary buffer ->
                            Binary buffer
